...
{    
    ...
    float *dev_ptr;
    cudaMalloc(&dev_ptr, NUM_ELEMENTS * sizeof(float));

    for(int i=0;i<NUM_ELEMENTS;i++) x[i] = i * 1.00f;
    std::cout << "Original array: "; 
    for(int i=0;i<NUM_ELEMENTS;i++) std::cout << x[i] << " ";

    cudaMemcpy(dev_ptr, x, NUM_ELEMENTS * sizeof(float), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(x, dev_ptr, NUM_ELEMENTS * sizeof(float), cudaMemcpyDeviceToHost);
