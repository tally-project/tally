...
float PRIORITY_MIN_WAIT_TIME_MS;
bool PRIORITY_USE_ORIGINAL_CONFIGS;
bool PRIORITY_USE_SPACE_SHARE;
float PRIORITY_WAIT_TIME_MS_TO_USE_ORIGINAL_CONFIGS;
...
void __attribute__((constructor)) register_env_vars()
{
    if (!TALLY_INITIALIZED) {
        ...
        REGISTER_FLOAT_ENV_VAR(PRIORITY_MIN_WAIT_TIME_MS, 0.f);
        REGISTER_FLOAT_ENV_VAR(PRIORITY_WAIT_TIME_MS_TO_USE_ORIGINAL_CONFIGS, 0.f);
        ...
        REGISTER_BOOL_ENV_VAR(PRIORITY_USE_ORIGINAL_CONFIGS, false);
        REGISTER_BOOL_ENV_VAR(PRIORITY_USE_SPACE_SHARE, false);
